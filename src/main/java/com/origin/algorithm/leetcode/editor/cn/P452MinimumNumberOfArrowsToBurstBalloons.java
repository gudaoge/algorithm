...
public class P452MinimumNumberOfArrowsToBurstBalloons {
    ...
    /**
     * 思路：
     * 我们每次射箭都要尽可能的刺破尽量多的气球
     * 因此我们每次射箭的时候都要从气球重叠最多的坐标射箭
     *
     * 即问题可以转换为区间重叠问题
     * 先对区间进行排序
     * 然后选定区间判断是否重叠，若重叠取交集
...
class Solution {
    public int findMinArrowShots(int[][] points) {
        ...
        while (index < points.length) {
            if (points[index][0] >= point[0] && points[index][0] <= point[1]) {
                //重叠 取交集
                point[0] = Math.max(point[0], points[index][0]);
                point[1] = Math.min(point[1], points[index][1]);
            } else {
                //不重叠
                ret++;
                point = points[index];

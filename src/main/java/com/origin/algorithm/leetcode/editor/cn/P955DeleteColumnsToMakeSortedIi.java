...
public class P955DeleteColumnsToMakeSortedIi {
    ...
    /**
     * 思路：
     * 对于第i列，若i-1列存在严格升序序列，则i列的顺序可以不做要求
     * 若存在相等元素的序列，则i列对应位置必须相等或递增
     * 因此采用一个数组保存前n列字符串的字典序大小，true表示大于上一行字符串，false表示相等
     */
    ...
    public int minDeletionSize(String[] A) {
        ...
        for (int i = 0; i < A[0].length(); i++) {
            ...
            for (int j = 1; j < A.length; j++) {
                if (!arr[j] && A[j].charAt(i) < A[j-1].charAt(i)) {
                    delete = true;
                    res++;
                    break;
                }
